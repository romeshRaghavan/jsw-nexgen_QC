...
function fetchBusinessExpNdEmployeeAdv() {
	alert("in fetchBusinessExpNdEmployeeAdv")
  j('#source').remove();
  mytable = j('<table></table>').attr({ id: "source",class: ["table","table-striped","table-bordered"].join(' ') });
	var rowThead = j("<thead></thead>").appendTo(mytable);
	var rowTh = j('<tr></tr>').attr({ class: ["test"].join(' ') }).appendTo(rowThead);
...
function fetchExpenseClaimFromMain() {
	alert("in fetchExpenseClaimFromMain");
...
try{
 var s = 
      ...
      +"<input class='abc' id='tripDate_"+i +"' type=button value="+tripDate+" onclick ='fetchTripDetails("+ i +","+requestTripId+");'>"
      +"</span>"
	  +"<div class='pull-left'>"
...
var request = "<li id = 'tripTravelId_"+i+"'>"
      +"<div class='trippreflist'>"
      +"<span class='pull-left'>"
      +"<input class='abc' id='travel_no_tr_"+i +"' type=button value="+travel_no_tr+" onclick ='fetchTicketPreferences("+ i +","+iternary_id_tr+")'>"
      +"</span>"
	  +"<div class='pull-left'>"
	  +"<table class='uptripstxttable'>"
...
function populatePriority(){
	...
	for(var i=0; i<flightpreferencesArrayNew.length; i++){
